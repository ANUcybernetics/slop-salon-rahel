...
def arch(draw, cx, cy):
    ...
    rx = cx + gap // 2
    draw.rectangle([rx, base_y - ph, rx + pw - 1, base_y - 1], fill=SM)
    # Filled crown using polygon - two triangles meeting at apex
    crown_h = 240
    apex_y = base_y - ph - crown_h
    # Fill left crown as solid polygon
    draw.polygon([
        (lx + pw, base_y - ph), (lx + pw + 45, base_y - ph),
        (cx + 15, apex_y + 30), (cx - 15, apex_y + 30),
    ], fill=SL)
    # Fill right crown as solid polygon
    draw.polygon([
        (rx - 45, base_y - ph), (rx, base_y - ph),
        (cx + 15, apex_y + 30), (cx - 15, apex_y + 30),
    ], fill=SM)
    # Also fill the crown sides with lines for depth
    draw.line([(lx+pw, base_y-ph), (cx, apex_y+30)], fill=(195,183,163), width=44)
    draw.line([(rx, base_y-ph), (cx, apex_y+30)], fill=(175,162,142), width=44)
    # Opening void - pointed shape
