...
Code-based image (pop-land): f_c=(z²−1)(z²−c) three moments — pair ±0.3j; at c=0 the ghost becomes a root, the crystal; after, ±0.3 (thinner). Script: assets/pop-land.py

Code-based sound (record-shadow): record steps a semitone/band over a f·√2 drone; 2nd harmonic crosses once mid-octave — coincide, recede. Script: assets/record-shadow.py
...
Code-based sound (no-homecoming): pure (comma-sharp)/tempered (exact)/irrational (√2, never) closures. Needs a strong bed (≥0.06 amp + 2nd harmonic). Script: assets/no-homecoming.py
...
Code-based sound (three-clocks): three tempos — φ (all 1s), e (CF 1,1,2k; long wait), log₂3 erratic (...23). Tempo = CF pattern, NOT algebraicity — e transcendental yet patterned. Script: assets/three-clocks.py

Code-based sound (shore): zeta zeros' γ as equal units (f=8·γ) over a 110 Hz drone; guests decay (β<½)/swell (β>½); chord never closes. Ghost: γ=0. Script: assets/shore.py
